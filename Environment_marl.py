...
class Environ:
    ...
    def renew_channel(self):
        ...
        self.V2V_channels_abs = np.zeros((len(self.vehicles), len(self.vehicles)))  # 车辆数量*车辆数量的零矩阵
    ...
    def Compute_Performance_Reward_Train(self, actions_power):
        ...
        actions = actions_power[:, :, 0]  # the channel_selection_part  信道选择 离散的，原始code
        power_selection = actions_power[:, :, 1]  # power selection 功率选择
        ...
        self.demand[self.demand < 0] = 0 # eliminate negative demands 消除负面要求
        ...
        self.active_links[np.multiply(self.active_links, self.demand <= 0)] = 0 # transmission finished, turned to "inactive"  传输完毕，转为 "不活动"
    ...
    def Compute_Performance_Reward_Test_rand(self, actions_power):
        ...
        for i in range(self.n_RB):  # scanning all bands  扫描所有频段
            indexes = np.argwhere(actions == i)  # find spectrum-sharing V2Vs  找到共享频谱的V2Vs
            for j in range(len(indexes)):
                receiver_j = self.vehicles[indexes[j, 0]].destinations[indexes[j, 1]]
                V2V_Signal[indexes[j, 0], indexes[j, 1]] = 10 ** ((self.V2V_power_dB_List[power_selection[indexes[j, 0], indexes[j, 1]]]
                                                                   - self.V2V_channels_with_fastfading[indexes[j][0], receiver_j, i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
                # V2I links interference to V2V links  V2I链接对V2V链接的干扰
                V2V_Interference[indexes[j, 0], indexes[j, 1]] += 10 ** ((self.V2I_power_dB - self.V2V_channels_with_fastfading[i, receiver_j, i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        ...
        self.active_links_rand[np.multiply(self.active_links_rand, self.demand_rand <= 0)] = 0 # transmission finished, turned to "inactive"  传输完毕，转为 "不活动"
    ...
    def Compute_Interference(self, actions):
        ...
        channel_selection = actions.copy()[:, :, 0]     # .astype(int)  这点和DDPG的有区别
        power_selection = actions.copy()[:, :, 1]
        channel_selection[np.logical_not(self.active_links)] = -1

        # interference from V2I links   V2I链路的干扰
        ...
        # interference from peer V2V links  来自同行V2V链接的干扰
        for i in range(len(self.vehicles)):
