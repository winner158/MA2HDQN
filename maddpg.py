...
with open('output_data.csv', 'w', newline='') as csvfile:
    ...
    csv_writer.writerow(['Step', 'V2I_rate', 'V2V_Rate', 'V2V_success', 'Remaining_V2V_payload'])
# CSV file path
output_file = 'output_data2.csv'
# Create a new CSV file and write the header
with open(output_file, 'w', newline='') as csvfile:
    csv_writer = csv.writer(csvfile)
    csv_writer.writerow(['Episode', 'Explore', 'Reward', 'Avg V2I Rate', 'Avg V2V Success'])
...
if __name__ == '__main__':
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):  # range内是0.1/0.001 = 100
            ...
            if i_episode < 10 or i_episode > n_episode - 10:
            ...
            v2i_rate_rounded = np.round(V2I_Rate, 2)
            v2v_rate_reshaped = np.round(V2V_Rate.reshape(4), 2)
            remaining_v2v_payload = np.round(env.demand.reshape(4), 2)
            with open('output_data.csv', 'a', newline='') as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow(
                    [i_step, ','.join(map(str, v2i_rate_rounded)), ','.join(map(str, v2v_rate_reshaped)), V2V_success,
                     ','.join(map(str, remaining_v2v_payload))])
        ...
        avg_v2i_rate = round(epsi_V2I_Rate / 100, 4)
        avg_v2v_success = round(epsi_V2V_success / 100, 4)
        # Append the data to the CSV file
        with open(output_file, 'a', newline='') as csvfile:
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(
                [i_episode, round(var, 4), round(sum_reward, 4), avg_v2i_rate, avg_v2v_success])
