...
with g.as_default():
    ...
    w_a = tf.Variable(tf.truncated_normal([n_hidden_3, n_output], stddev=0.1))
    w_v = tf.Variable(tf.truncated_normal([n_hidden_3, 1], stddev=0.1))
    ...
    b_a = tf.Variable(tf.truncated_normal([n_output], stddev=0.1))
    b_v = tf.Variable(tf.truncated_normal([1], stddev=0.1))
    ...
    a = tf.add(tf.matmul(layer_3_b, w_a), b_a)
    v = tf.add(tf.matmul(layer_3_b, w_v), b_v)

    y = v + a - tf.reduce_mean(a, axis=1, keepdims=True)
